...
function ListComponent() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setProductList(Array.isArray(list.data) ? list.data : []);
            } catch (error) {
    ...
    return (
        <>
            ...
            <Link className="btn btn-success " style={{color: 'white', textDecoration: 'none'}} to={'/home/create'}>Thêm
                mới</Link>
            <table className="table table-striped">
                ...
                {productList && productList.map((e, i) => (
                    <tr key={i}>
                        <td>{i + 1}</td>
                        <td>
                            <Link to={'/home/detail/' + e.id} style={{textDecoration: 'none'}}>{e.title}</Link>
                        </td>
                        <td>{e.description}</td>
                        <td>{e.price}</td>
                        <td>
                            <button type="button" className="btn btn-danger " style={{color: "white"}}
                                    onClick={() => handleShowDeleteModal(e)}>Xóa
                            </button>
